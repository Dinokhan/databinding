<script>
    ...
    var material = {
        Recordmaterial: ko.mapping.fromJS(model.kelasModel),
        Listmaterial: ko.observableArray([]),
        Mode: ko.observable(''),
        FilterText: ko.observable(''),
        DataFilter: ko.observableArray(['Kelas']),
        FilterValue: ko.observable('Kelas'),
    }
    material.selectdata = function(id) {
        $.ajax({
            url: "<?php echo site_url('Kelascontroller/getDataSelect') ?>",
            type: 'post',
            dataType: 'json',
            data : {id: id},
            ...
            <div class="x_content" data-bind="with:material">
                ...
                <div class="row" data-bind="visible:Mode() === ''">
                    <div class="col-md-12 padding-filter">
                        <div class="col-md-5">
                            <button class="btn btn-sm btn-primary" data-bind="click:add"><span class="glyphicon glyphicon-plus"></span> Add Kelas</button>
                        </div>
                        <div class="col-md-12">
                    ...
                    <div class="col-md-12">
                        <table id="grIdKelas" cellpadding="0" cellspacing="0" width="100%" class="table table-bordered table-striped" id="tableik">
                            <thead>
                                <tr>
                                    <th>Kelas</th>
                                    <th>Harga</th>
                ...
                <div class="row" data-bind="visible:Mode() === 'Update' || Mode() === 'Save'">
                    ...
                    <div class="col-md-12" data-bind="with:Recordmaterial">
                        <div class="col-md-6">
                            ...
                            <div class="col-md-8">
                                <input name="txtmaterialname" class="form-input form-control" type="text" data-bind="value:Kelas" />
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="col-md-4 filter-label">Harga</label>
                            <div class="col-md-8">
                                <input name="txtmaterialname" class="form-input form-control" type="number" data-bind="value:Harga" />
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    material.backForm = function(){
        material.Mode('');
        ko.mapping.fromJS(model.kelasModel,material.Recordmaterial);
    }
    ...
    material.save = function(){
        var url = "<?php echo site_url('Kelascontroller/save') ?>";
        if(material.Mode() === 'Update')
            url = "<?php echo site_url('Kelascontroller/update') ?>";
    ...
    material.remove = function(id){
        $.ajax({
            url: "<?php echo site_url('Kelascontroller/delete') ?>",
            type: 'post',
            dataType: 'json',
            data: {id: id},
            success : function(res) {
    ...
    $(document).ready(function () {
        material.grid = $("#grIdKelas").DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": "<?php echo site_url('Kelascontroller/getData') ?>",
                "type": "POST",
                "data": function(d){ 
